...
import { listDevNotifications, subscribeDevData } from '../../demo/devDataStore';
...
const category = (kind:string):Tab => kind.includes('chat')?'message':kind.includes('request')?'system':kind.includes('shipment')||kind.includes('delivered')||kind.includes('hold')||kind.includes('payment')||kind.includes('released')||kind.includes('delayed')||kind.includes('cancelled')||kind.includes('terminated')?'shipment':'system';
...
export default function AdminNotifications() {
  ...
  // Development data store events (lifecycle actions on development shipments) are listed too.
  const [devVersion,setDevVersion]=useState(0);
  useEffect(()=>subscribeDevData(()=>setDevVersion(value=>value+1)),[]);
  const allDeliveries=useMemo(()=>[...listDevNotifications().map(note=>({id:note.id,tracking_id:note.shipmentId,recipient_email:'',recipient_type:'development',notification_type:note.type,delivery_status:'recorded',attempt_count:0,sent_at:note.createdAt,error_summary:note.message,created_at:note.createdAt} as Delivery)),...deliveries].sort((a,b)=>(b.sent_at||b.created_at).localeCompare(a.sent_at||a.created_at)),[deliveries,devVersion]); // eslint-disable-line react-hooks/exhaustive-deps
  const visible=useMemo(()=>allDeliveries.filter(delivery=>tab==='all'||category(delivery.notification_type)===tab),[allDeliveries,tab]);
  const counts=useMemo(()=>({all:allDeliveries.length,shipment:allDeliveries.filter(item=>category(item.notification_type)==='shipment').length,message:allDeliveries.filter(item=>category(item.notification_type)==='message').length,system:allDeliveries.filter(item=>category(item.notification_type)==='system').length}),[allDeliveries]);
  const reference=(delivery:Delivery)=>{const shipment=shipmentById.get(delivery.tracking_id);return shipment?referenceOf(shipment):delivery.tracking_id?delivery.tracking_id.slice(0,8).toUpperCase():'Request';};
